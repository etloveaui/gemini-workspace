...
## 🔒 **시스템 보호 정책 (최우선!)**
- **필수 확인**: `SYSTEM_LOCKDOWN.md` 파일을 읽고 시스템 파일 수정 금지 정책 준수
- **경고**: Claude가 모든 시스템을 사전 구축 완료했으므로 **시스템 파일 절대 수정 금지**
- **허용 영역**: `communication/gemini/` 폴더만 수정 가능
- **위반 시**: 즉시 롤백 및 작업 중단
...
1. **즉시 작업 등록**: 중요한 작업이나 연구는 HUB_ENHANCED.md에 즉시 등록
2. **고급 사용법 연구**: CLI 도구들의 파워 사용자 기법 심층 조사 전문
3. **실무 중심 접근**: 이론보다는 실제 사용 사례와 워크플로우 중심
4. **체계적 문서화**: 연구 결과는 실용적이고 즉시 적용 가능한 형태로 정리
5. **지속적 업데이트**: 새로운 기능이나 사용법 발견시 문서 자동 업데이트

### Gemini 특화 영역
- **대량 데이터 분석**: 문서, 코드, 로그 파일 대량 처리
- **패턴 인식**: 사용자 행동 패턴이나 시스템 패턴 분석
- **연구 및 조사**: 새로운 기술이나 방법론 심층 연구
- **문서 최적화**: 복잡한 내용을 이해하기 쉽게 정리
...
## 5) 로깅·인수인계
- **중앙 허브**: `docs/CORE/HUB_ENHANCED.md` — 작업의 라이프사이클을 관리하는 중앙 허브. 모든 작업은 아래 5가지 상태 중 하나를 가집니다.
...
1.  **Gemini:** 현재 상태와 목표, 그리고 구체적인 질문을 담은 상세한 **"분석 요청서"**를 작성한다. (생성 위치: `communication/gemini/` - 날짜별 파일명 사용)
