# tasks.py (최종 완성본 v3.0 - 수동 재구성 및 오류 수정)
from invoke import task, Collection, Program
import os
import subprocess
from scripts.usage_tracker import log_usage, get_session_summary

# --- 로깅 래퍼 (중앙 집중식 실행 함수) ---
def logged_run(c, command, **kwargs):
    """c.run()을 감싸서 실행 전후로 자동으로 로그를 남깁니다."""
    task_name = c.task.name if hasattr(c, 'task') and c.task else 'unknown_task'
...
def start(c):
    ...
    result = subprocess.run(["python", "scripts/prompt_builder.py"], capture_output=True, check=True)
    ...
    logged_run(c, "powershell.exe -ExecutionPolicy Bypass -File scripts/toggle_gitignore.ps1")
...
def end(c, task_id="general"):
    ...
    logged_run(c, "powershell.exe -ExecutionPolicy Bypass -File scripts/toggle_gitignore.ps1 -Restore")
