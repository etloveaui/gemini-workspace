...
static void TLF35584_DebugErrorResetCheck(void)
{
	...
	if ((currentTime - lastInitResetTime) > DEBUG_INIT_TIMEOUT_TICK) {
...
void TLF35584_HandleState()
{
	...
	case TLF35584_STATE_INIT:
		...
		if(!vTLF35584_Panels.error_control.error_flag &&
				vTLF35584_Panels.state_machine.Cnt.init > INIT_TO_NORMAL_DELAY_TICK){
	...
	case TLF35584_STATE_NORMAL:
		...
		if(!vTLF35584_Panels.error_control.error_flag) {
			if(!vTLF35584_Panels.read_pins.ENA && !vTLF35584_Panels.read_pins.WAK){
				if((vTLF35584_Panels.sleep_count > IG_OFF_SLEEP_DELAY_TICK) || (vTLF35584_Panels.force_shutdown_request == TRUE)){
					vTLF35584_Panels.sleep_count = 0;
					vTLF35584_Panels.force_shutdown_request = FALSE; // Reset the flag
				...
				vTLF35584_Panels.sleep_count = 0;
				vTLF35584_Panels.force_shutdown_request = FALSE;
			}
		}
		else{
			//no action
